...
namespace Util {
        ...
        const uint32_t num_bits)
    {
        ...
        uint64_t tmp = 0;
        const auto nbytes = ((start_bit + num_bits + 7) >> 3);
        const auto max_bytes = sizeof(tmp);
        const auto num_bytes = nbytes > max_bytes ? max_bytes : nbytes;
        memcpy(&tmp, bytes, num_bytes);
